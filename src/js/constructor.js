(function($){
	$(document).ready(function() {
		...
		var paperConfig = [
			...
			{ name: 'Лайнер', 					color: 'brown', 			density: [], 													laminating: true },
		];

		var themePath = '';
		var ajaxUrl = '';
		try{
			themePath = WP_variables.theme_path + '/';
			ajaxUrl = WP_variables.ajax_url;
		}catch(err){
			console.log(err);
		}
		var boxConfig = {
			...
			texturesPath: themePath + 'images/textures/',
			textures: {
				bg: 				false,
				bg_alpha: 			false,
				bg_laminating: 		false,
		...
		function generateLetterText(data){
			...
			data.groups.map(function(group){
				if( group.items.length > 1 ){
					...
					text += group.label.toUpperCase() + ': ' + group.items[0].value + '\r\n';				
				}
			});
			return text;
		}
		...
		$('#mark').each(function(){
			...
			$(this).change(function(){
				...
				var $results = $(this).data().select2.results.$results;
				if( hidden ){
					setTimeout(function(){
						$results.find('li[id$='+hidden+']').hide();					
					}, 500);
				}
			});
		});
		...
		$('#laminating').change(function(){
			...
			if( isChecked ){
				$('#mark').val( $('#mark').data('hidden') ).trigger('change');
			}else{
		...
		$('#metal_cardboard').change(function(){
			...
			$('#solid_polish_no').prop('checked', isChecked);	
		});
		...
		$('#constructor_send').click(function(evt){
			...
			$('#send_params').val( generateLetterText(data) );
			eupacking.toggleModal( 'modalConstructorSend' );
		});

		//Сохранить параметры
		$('#constructor_save').click(function(evt){
			evt.preventDefault();
			var data = getFormData();
			var tableHtml = generateTable( data );
